...
- All-in-one container: PHP 8.3-FPM + nginx + MariaDB + s6-overlay on Alpine
- **This is the Flarum 1.x edition branch** — see "Branches & Editions" below
...
- `main` = **Flarum 2.x edition** — PHP 8.5, `mariadb` driver key, 80+ extensions → git tags `v0.x.y` → Docker `latest`, `0.x`
- `1.x` = **Flarum 1.x edition** (this branch) — PHP 8.3, `mysql` driver key, 40+ curated extensions → git tags `v0.1.y` → Docker `0.1.y`, `0.1`, `flarum1`. **Never publishes `latest`** — that tag always points at the 2.x edition from `main`.
- The 1.x edition is deliberately lean: all bundled `flarum/*` extensions plus a curated community set. The composer resolver is the 1.x-compatibility oracle — anything that fails resolution goes into `_disabled-require` or gets dropped.
- **Default-enabled set mirrors main**: never enable in `extensions.txt` what the 2.x edition doesn't enable by default (installed-but-disabled is fine). Bundled `flarum/*` extensions are enabled by the Flarum installer itself (`EnableBundledExtensions`, incl. approval) and aren't listed — `flarum-approval` is listed explicitly as a guarantee.
...
- Flarum 1.x quirks on this branch:
  - The DB `driver` must be `mysql` in **both** `src/box/data/install.yaml` and `src/box/config.php` — the `mariadb` driver key doesn't exist until Flarum 2.x (it still connects to MariaDB via the unix socket just fine)
  - The tags table has **no `is_primary` column** — primary vs secondary tags are modeled by the `parent_id` hierarchy (`parent_id IS NULL` = primary). See `tags.tsv` + `import-tabular.php`
  - A YAML install does **not** enable bundled extensions (1.x `FileDataProvider` turns a missing `extensions:` key into `['']`; 2.x passes `null` and falls back to the whitelist) — `extensions.txt` must list the bundled `flarum/*` whitelist explicitly, and `flarum-flags` must precede `flarum-approval` (approval requires flags)
  - `justoverclock/flarum-ext-welcomebox` depends on the "Forum Widgets" extension, which **is** the package `afrux/forum-widgets-core` (its extension ID is `afrux-forum-widgets-core` — there is no `afrux/forum-widgets` package)
  - `linkrobins/auto-verify` is 2.x-only — the 1.x image ships a local demo helper (`src/box/extensions/demo-auto-confirm`, installed via a composer path repo) that auto-confirms signups on the `Registered` event instead
  - PHP logging is hardened on this image (`display_errors=0`): Flarum 1.x's abandoned `swiftmailer` emits deprecation notices under PHP 8.3 that would otherwise leak into responses and break them mid-request. PHP errors go to `storage/logs/php-error.log`
  - **Flamoji** ships its 1.x line (`pianotell/flarum-ext-flamoji` v1.5.x) and must stay in the list
  - The Flarum 1.x line is upstream maintenance mode (critical/security fixes only, EOL ~end of 2026) — expect this branch to be short-lived
  - No Docker Hub documentation sync on this branch (`dockerhub-description.yml` removed); the Hub description is owned by `main`
...
- **CI builds only on tag push** (`v0.1*` on this branch) and manual dispatch — pushes to the `1.x` branch don't trigger builds. To release: `git tag v0.1.y && git push --tags`
...
- Tags published to Docker Hub: `0.1.y`, `0.1`, `flarum1`, `sha-...` (never `latest`)
- No Docker Hub description sync on this branch — README/docs here are repo-only; the Hub description is synced from `main` only.

---

## Docker / Build

- The `php:8.3-fpm-alpine` base does NOT have prebuilt extensions for our PHP version — `docker-php-ext-install` compiles from source. This is unavoidable; GHA layer cache (`type=gha`) handles it for subsequent builds. (The 2.x edition on `main` uses `php:8.5-fpm-alpine` — Flarum 1.x supports PHP up to 8.3, hence the older base here.)
...
- [ ] Count guidance string (currently `40+`) must remain accurate. If total composer-installed extensions crosses a tens-boundary, update **all 4 places** (see "Extension count consistency" below).
...
The count phrase (`40+ extensions`) appears in **4 distinct files**. When
...
(The 2.x edition on `main` has its own 8-place `80+` map.)
...
### Files to update (current count phrase: `40+`)

| File | Where |
|------|-------|
| `README.md` | Tagline (line ~5): `Flarum 1.8, 40+ extensions, PHP 8.3, MariaDB, and nginx` |
...
(The README also carries one intentional `80+` cross-reference pointing
2.x-curious readers at the `latest` tag — that one stays.)

### Rules for the count phrase

- Use a round friendly number (`40+`, not `47` or `~40`)
...
- Never use `~30` / `~40` historical tilde phrasings — replace any stale tilde-form references found

---

## When BUMPING the Flarum core version

- [ ] Update `composer.json.TEMPLATE` `flarum/core` constraint if pinned (otherwise leave `^1.8`)
...
Mermaid (`datlechin/flarum-mermaid`) is a Flarum 2.x-only extension and is
**not shipped on this branch** — the seed discussions here contain no
Mermaid samples. If a 1.x-compatible release lands, add it back along with
samples and update this section.

---

## Testing

- After build, verify: HTTP 200 at the mapped port, all 7 seed discussions present, sticky posts pinned, no `ERROR` entries in `/var/www/html/storage/logs/*.log`.
...
# (stale = 10–79 or 100+; `40+` is the current count phrase on this branch)
grep -rnE '~?\\?~?([1-3][0-9]|[5-9][0-9])[0-9]*\+?\s*(popular |demo )?extensions?' \
    --include='*.md' --include='*.json' --include='*.yml' \
    --include='*.yaml' --include='Dockerfile' --include='*.TEMPLATE' \
    | grep -v 'Initial release: all-in-one' \
    | grep -v 'Flarum 2.x edition'
...
orphans = enabled - {to_id(p) for p in installed} - {'flarum-extension-manager', 'afrux-forum-widgets-core'}
