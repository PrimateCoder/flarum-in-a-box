...
- **CHANGELOG entries**: extreme brevity. One short line per change is ideal. Don't list affected files, paths, byte counts, root causes, "previously X bytes / now Y bytes" comparisons, or implementation details. Users only need to know what changed at a high level. **No `### Added`/`### Changed`/`### Fixed` subsections** — just a flat bullet list under the version header. **No parenthetical justifications** ("(was cluttering X)", "(official spelling)") — if it's worth saying, say it briefly without parens; usually it's not. **Only user-facing changes belong in the CHANGELOG.** Skip internal cleanups (removing dead code/redundant build steps), README/docs tweaks, and anything that doesn't change behavior or appearance for someone running the image.
