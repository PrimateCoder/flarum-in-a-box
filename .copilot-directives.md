...
- The first boot should be ~8s (DB init step is gone — just `s6-overlay` starting nginx/php-fpm/mariadb).
...
- **CHANGELOG entries**: extreme brevity. One short line per change is ideal. Don't list affected files, paths, byte counts, root causes, "previously X bytes / now Y bytes" comparisons, or implementation details. Users only need to know what changed at a high level. **Use Keep-a-Changelog subsections** (`### Added`, `### Changed`, `### Fixed`, `### Removed`) under each version heading — they group related changes and stay consistent with the older entries in this file. **No parenthetical justifications** ("(was cluttering X)", "(official spelling)") — if it's worth saying, say it briefly without parens; usually it's not. **Only user-facing changes belong in the CHANGELOG.** Skip internal cleanups (removing dead code/redundant build steps), README/docs tweaks, and anything that doesn't change behavior or appearance for someone running the image. **Version heading format**: `## X.Y.Z — YYYY-MM-DD` (em dash + ISO date), matching the convention used in PrimateCoder's other Flarum repos (e.g., flarum-flamoji).
- **nginx** is always lowercase (it's the official spelling).
