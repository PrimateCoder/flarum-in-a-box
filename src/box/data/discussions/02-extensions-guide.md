...
- 🥑 [**Avocado**](https://discuss.flarum.org/d/38940-avocado-theme) — Modern, polished theme with hero banner and rich customization (see the dedicated post!)
...
- 🖼️ [**Discussion Thumbnail**](https://packagist.org/packages/fof/discussion-thumbnail) — Thumbnails in discussion list
...
### Try the Moderation Tools

This instance also ships with a **🛠️ Moderation Playground** discussion (with users `user1` through `user5` arguing about moderation policy) and a dedicated `moderator` account (in the Mod group). Log in as `moderator` / `password` and practice locking, hiding, suspending, etc. on real-looking content. Combine with [**Log Viewer**](https://packagist.org/packages/ianm/log-viewer) to see your moderator actions logged.
